...
def report_df_processing(user_df: pd.DataFrame, report_query) -> pd.DataFrame:
    ...
    user_department_mapping = dict(zip(user_df['id'], user_df['department']))
    ...
    def replace_ids_with_departments(ids, mapping):
        """Goes through each user_id and maps to department"""
        if not ids:  # Handle None or empty values
            return ''
        try:
            # Split the string into a list of IDs
            id_list = map(int, ids.split(','))
            # Replace each ID with the corresponding name
            department_list = [mapping.get(user_id, f"Unknown ID: {user_id}") for user_id in id_list]
            # Join the names back into a single string
    ...
    if 'question' in report_df.columns:
        report_df['creator_name'] = report_df['tqask_created_by_id'].map(user_mapping)

        report_df['creator_department'] = report_df['tqask_created_by_id'].map(user_department_mapping)

        report_df['creator_email'] = report_df['tqask_created_by_id'].map(user_email_mapping)
    ...
    report_df.drop(columns=['contract_owner', 'created_by_id', 'last_updated_by_id', 'tqask_created_by_id'], errors='ignore', inplace=True)
